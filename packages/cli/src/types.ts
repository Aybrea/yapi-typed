...
export type TemplateRenderer = (ctx: TemplateContext) => AsyncOrSync<string>

export type TemplateInput = string | TemplateRenderer
...
export interface TemplateConfig {
   ...
   * 请求函数文件模板。
   */
  requestFunction?: TemplateInput
  /**
   * 请求 Hook 生成器模板。
   */
  requestHookMaker?: TemplateInput
  /**
   * 生成文件头部模板。
   */
  fileBanner?: TemplateInput
}

export interface FormatConfig {
  /**
   * 是否使用 Prettier 格式化生成文件。
   *
   * @default true
   */
  prettier?: boolean
  /**
   * 额外的 Prettier 配置，会与项目配置合并。
   */
...
/** 配置输入。 */
export type Config = RootConfig | ServerConfig | ServerConfig[]
