...
import { Defined, OneOrMore } from '../utils/vtils-types'
...
export function configureSchemaCache(options?: { enabled?: boolean; max?: number }) {
  ...
  if (typeof options.max === 'number' && options.max > 0) {
    schemaTypeCache = new LRUCache<string, string>({ max: options.max })
  }
}
...
export async function jsonSchemaToType(
  ...
  const cacheKey = schemaCacheEnabled
    ? createSchemaCacheKey(jsonSchema, typeName)
    : ''
  if (schemaCacheEnabled && schemaTypeCache.has(cacheKey)) {
    return schemaTypeCache.get(cacheKey) as string
  }
  ...
  const result = code.replace(fakeTypeName, typeName).trim()
  if (schemaCacheEnabled) {
    schemaTypeCache.set(cacheKey, result)
  }
  return result
}
